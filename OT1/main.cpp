...
//找到当前nfa的终态
char findend(char a,set<char> x) {
	if (nfa[a].empty()) return a;
	else {
		x.insert(a);
		int i = 0;
		//返回字符集合 x 中与 nfa[a][i].next 匹配的元素的迭代器。
		//如果没有匹配项，它将返回 x.end()，表示没有找到匹配。
		while (x.find(nfa[a][i].next) != x.end()) {
			i++;
		}
		char t=findend(nfa[a][i].next,x);
		return t;
	}
}
...
int main() {
	...
	printf("终态集为：");
	set<char>x;
	cout << findend(s,x) << endl;
	out << findend(s, x) << endl;
	printf("转移函数为：\n");
	getfunction();
